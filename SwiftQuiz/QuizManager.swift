...
class QuizManager {
    ...
    var totalResposta : Int {
        return _totalRespostas
    }
    
    var totalCorretas : Int{
        return _totalCorretas
    }
    func refreshQuiz(){
        let randomIndex = Int (arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes [randomIndex]
        quiz = Quiz (question: quizData.question, option: quizData.options , respostaCorreta: quizData.correctAnswer)
        
    }
    func validateResposta(index : Int){
        _totalRespostas += 1
